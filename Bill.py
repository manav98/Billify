...
class Bill_App:
    ...
    def welcome_bill(self):
        ...
        self.txtarea.insert(END, "\tWelcome To This Firm\n")
    ...
    def bill_area(self):
        if self.c_name.get() == "" or self.c_phone == "":
            messagebox.showerror("Error", "Invalid Customer Details")
            ...
            if self.soap.get() != 0:
                self.txtarea.insert(
                    END, f"\n Bathing Soap\t\t{self.soap.get()}\t\t{self.c_s_p}"
                )
            ...
            if self.thumsup.get() != 0:
                self.txtarea.insert(
                    END, f"\n ThumsUp\t\t{self.thumsup.get()}\t\t{self.d_f_p}"
                )
            if self.limca.get() != 0:
                self.txtarea.insert(END, f"\n Limca\t\t{self.limca.get()}\t\t{self.d_t_p}")
    ...
    def find_bill(self):
        ...
        for i in os.listdir("Bills/"):
            if i.split('.')[0]==self.search_bill.get():
                ...
                self.txtarea.delete('1.0', END)
                for itr in f1:
                    self.txtarea.insert(END, itr)
                f1.close()
                present = TRUE
            if present == FALSE:
                messagebox.showerror("Error", "Invalid Bill Number")
    def clear_data(self):
        op = messagebox.askyesno("Clear", "Do your really want to Clear Data?")
        if op == TRUE:
            # ===================================== VARIABLES ==============================================================
            #   CUSTOMER
            self.c_name.set("")
    ...
    def Exit_app(self):
        op = messagebox.askyesno("Exit", "Do your really want to exit?")
        if op == TRUE:
            self.root.destroy()
